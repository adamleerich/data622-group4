...
The `randomForest` function has dozens of parameters,
but the two most important are

* `mtry`, the number of variables to randomly select for each tree
* `ntree`, the number of trees to build
* `strata` and `sampsize` to control sampling

The function comes with sensible defaults, so we run those
to get a baseline.
...
The reader will notice that the "No. of variables at each split"
in these two models is different.  The `mtry` parameter determines
this value and it defaults to the square root of the number of columns.
The data set with dummy variables has more columns, so its default
value for `mtry` is higher.

Both of these models have an error rate of nearly 20% overall,
but this is driven by the class error rate for `Loan_Status` = "N".
This is probably because of an imbalance in our samples.

```{r}
plot(m1)
legend("topright", colnames(m1$err.rate), col = 1:4, fill = 1:4)
```
...
### Model #2: stratified sampling to balance `Loan_Status`
