...
# Loads these objects from #2
#   loans_transformed_test
#   loans_transformed_train
load('data/loans_transformed_test-train.RData')

# xgboost wants predictors and response in separate objects
train_labels <- ifelse(
  loans_transformed_train$Loan_Status == 'Y', 1, 0)

test_labels <- ifelse(
  loans_transformed_test$Loan_Status == 'Y', 1, 0)
...
# TODO Delete!
loans_dv <- rbind(loans_transformed_test, loans_transformed_train)
mloans <- as.matrix(loans_dv)

# xgboost can use a special xgb.DMatrix object
dtrain <- loans_transformed_train %>% 
  select(-Loan_Status) %>% 
  as.matrix %>% 
  xgb.DMatrix(
    data = .,
